...
def preprocess_single_inputs(tokenizer: transformers.PreTrainedTokenizer, inputs: List[str], max_length=256, use_chat_template=False, device='cuda'):
    """
    ...
    # else:
    # FIXME: this is a hack to truncate the inputs to max_length, without truncating the generation tokens
    new_inputs = []
    for example in inputs:
        if len(example.split()) > max_length - 128 - 50:
            print(len(example.split()))
            example = ' '.join(example.split()[:max_length - 128 - 50])
...
def main(args):
    ...
    if total_samples > len(prompts):
        prompts = prompts * (total_samples // len(prompts) + 1)
    prompts_this_process = prompts[rank::dist.get_world_size()]
    ...
    for idx in pbar:
        ...
        y = [f"Please generate an image of {prompts_this_process[i]}" for i in range(idx * n, (idx+1) * n)]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prompts-path", type=str, help="Path to prompts file (.jsonl)")
